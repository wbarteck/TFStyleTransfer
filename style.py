    ...
    with tf.Graph().as_default():
        ...
        for i in range(len(styles)):
            ...
            style_loss += style_weight * \
                style_blend_weights[i] * reduce(tf.add, style_losses)
        ...
        train_step = tf.train.AdamOptimizer(
            learning_rate, beta1, beta2, epsilon).minimize(loss)
        ...
        with tf.Session() as sess:
            ...
            for i in range(iterations):
                ...
                if (checkpoint_iterations and i % checkpoint_iterations == 0) or last_step:
                    ...
                    img_out = vgg.unprocess(
                        best.reshape(shape[1:]), vgg_mean_pixel)
                    ...
                    if preserve_colors and preserve_colors == True:
                        ...
                        # 2
                        styled_grayscale_yuv = np.array(Image.fromarray(
                            styled_grayscale_rgb.astype(np.uint8)).convert('YCbCr'))

                        # 3
                        original_yuv = np.array(Image.fromarray(
                            original_image.astype(np.uint8)).convert('YCbCr'))
                        ...
                        # 5
                        img_out = np.array(Image.fromarray(
                            combined_yuv, 'YCbCr').convert('RGB'))
                else:
                    img_out = None
